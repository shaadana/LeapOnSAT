...
export default function ClassCalendar({ classId, user }) {
  ...
  // Class events for THIS class that are visible to this student + the student's own availability
  const { data: allEvents = [] } = useQuery({
    queryKey: ['classCalendarEvents', classId, user?.id],
    queryFn: async () => {
      const classEvents = await base44.entities.CalendarEvent.filter(
        { event_type: 'class', class_id: classId },
        '-created_date',
        1000
      );
      const visibleClassEvents = classEvents.filter(
        (e) => !e.visible_to_students || e.visible_to_students.length === 0 || e.visible_to_students.includes(user.id)
      );
      const myAvailability = await base44.entities.CalendarEvent.filter(
        { user_id: user.id, event_type: 'availability' },
        '-created_date',
        1000
      );
      const combined = [...visibleClassEvents, ...myAvailability];
  ...
  const { data: myRsvps = [] } = useQuery({
    queryKey: ['myClassRsvps', user?.id, classId],
    queryFn: async () => base44.entities.CalendarRSVP.filter({ student_id: user.id, class_id: classId }, '-created_date', 500),
    enabled: !!user?.id && !!classId,
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data) =>
      base44.entities.CalendarEvent.create({ ...data, user_id: user.id, class_id: classId, visible_to_students: [] }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['classCalendarEvents', classId, user?.id] });
      setShowDialog(false);
      setFormData({ title: '', description: '', event_type: 'availability', start_time: '', end_time: '' });
  ...
  const rsvpMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['myClassRsvps', user?.id, classId] });
  ...
  const getTypeLabel = (type) => {
    switch (type) {
      ...
      case 'class': return 'Scheduled Class';
      default: return type;
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold text-stone-900">Class Calendar</h2>
        </div>

        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogTrigger asChild>
            <Button className="bg-emerald-500 hover:bg-emerald-600 rounded-full font-bold">
              <Plus className="w-5 h-5 mr-2" /> Add Availability
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
        ...
        <div className="md:col-span-1">
          ...
          <div className="mt-4 flex flex-col gap-2 pl-2">
            <div className="flex items-center gap-2 text-xs text-stone-600">
              <div className="w-3 h-3 rounded-md bg-emerald-600" /> Scheduled Classes
            </div>
            <div className="flex items-center gap-2 text-xs text-stone-600">
