...
import { SAT_GRAPH_NODES, DOMAIN_COLORS } from '@/data/satKnowledgeGraph';
...
// Grid layout: each domain gets its own horizontal band; nodes sorted by depth then placed in clean rows
function computeLayout(nodes) {
  ...
  const DOMAIN_ORDER = ['Foundations', 'Algebra', 'Advanced Algebra', 'Statistics', 'Geometry', 'Trigonometry', 'Problem Solving'];
  const byDomain = {};
  nodes.forEach(n => {
  ...
  const COLS = 7;          // nodes per row within a domain
  const COL_W = 180;       // horizontal spacing
  const ROW_H = 140;       // vertical spacing between rows within a domain
  const DOMAIN_GAP = 60;   // extra vertical gap between domains
  ...
  // Any leftover nodes not in DOMAIN_ORDER
...
export default function ConceptGraph({ nodes, onNodeClick, selectedNodeId }) {
  ...
  // If nodes are already fully merged (have prerequisites array from static data), use them directly.
  // Otherwise fall back to merging raw DB nodes with SAT_GRAPH_NODES.
  const MASTERY_ORDER_LIST = ['not_started', 'learning', 'practiced', 'mastered'];
  const isPreMerged = (nodes || []).length > 0 && (nodes[0]?.prerequisites !== undefined);

  let graphNodes;
  if (isPreMerged) {
    graphNodes = nodes;
  } else {
    const deduplicatedByNodeId = {};
    (nodes || []).forEach(n => {
      const key = n.node_id || n.title;
      if (!key) return;
      const existing = deduplicatedByNodeId[key];
      if (!existing) {
        deduplicatedByNodeId[key] = n;
      } else {
        const existingIdx = MASTERY_ORDER_LIST.indexOf(existing.mastery_level || 'not_started');
        const newIdx = MASTERY_ORDER_LIST.indexOf(n.mastery_level || 'not_started');
        if (newIdx > existingIdx) deduplicatedByNodeId[key] = n;
      }
    });
    graphNodes = SAT_GRAPH_NODES.map(staticNode => {
      const userNode = deduplicatedByNodeId[staticNode.id] || deduplicatedByNodeId[staticNode.title];
      return {
        ...staticNode,
        mastery_level: userNode?.mastery_level || 'not_started',
  ...
  useEffect(() => {
    ...
    const pos = computeLayout(graphNodes);
    setPositions(pos);
  }, []);
  ...
  const renderEdge = ({ from, to }) => {
    ...
    const R = 36; // node radius approximation
    const dx = p2.x - p1.x;
    const dy = p2.y - p1.y;
    ...
    return (
      <path
        ...
        opacity={0.8}
        markerEnd={bothMastered ? 'url(#arrow-green)' : prereqMastered ? 'url(#arrow-light)' : 'url(#arrow-gray)'}
      />
    );
  };
  ...
  const renderNode = (node) => {
    ...
    if (!pos) return null;
    const domainColor = DOMAIN_COLORS[node.domain] || { base: '#6b7280', light: '#f3f4f6', dark: '#374151' };
    ...
    const isSelected = selectedNodeId === node.id || selectedNodeId === node.dbId;
    const R = 38;

    // Determine if unlocked (all prerequisites mastered)
    ...
    return (
      <g
        ...
        onClick={(e) => { e.stopPropagation(); onNodeClick({ ...node, node_id: node.id }); }}
      ...
      >
        ...
        <circle
          ...
          cy={-(R - 6)}
          r={7}
          fill={MASTERY_STROKE[node.mastery_level] || '#94a3b8'}
          stroke="white"
          strokeWidth={1.5}
        />
        {/* Domain label pill below */}
        ...
        <g transform={`translate(${transform.x},${transform.y}) scale(${transform.scale})`}>
          ...
          {(() => {
            ...
            return DOMAIN_ORDER.map(domain => {
              ...
              const xs = nodesInDomain.map(n => positions[n.id]?.x).filter(x => x !== undefined);
